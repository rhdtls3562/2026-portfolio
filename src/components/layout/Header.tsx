/** 상단 고정 네비게이션과 모바일 메뉴 토글을 담당하는 헤더 컴포넌트. */
"use client";
  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? "bg-(--color-bg)/95 backdrop-blur-sm border-b border-(--color-border)"
          : "bg-transparent",
      )}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 h-16 md:h-20 flex items-center justify-between">
        ...
        >
          <span
            className={cn(
              "block w-6 h-px bg-(--color-text-primary) transition-all duration-300",
      ...
      >
        <nav className="flex flex-col border-t border-(--color-border) bg-(--color-bg)/95 px-6 pb-6 backdrop-blur-sm">
          {NAV_ITEMS.map((item) => (
            <button
              key={item.href}
              onClick={() => handleNavClick(item.href)}
